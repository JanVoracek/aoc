...
function findOptimalRoute(input: string) {
  ...
  while (queue.size() > 0) {
    ...
    if (step.position.x === end.x && step.position.y === end.y) {
      if (step.stepsInDirection < 4) continue;
      return step.heatLoss;
    }

    const nextPositions = getNextPositions(step.position, step.direction)
      .filter(([_, direction]) => {
        if (step.stepsInDirection < 4) return direction === step.direction;
        if (step.stepsInDirection > 9) return direction !== step.direction;
        return true;
      })
